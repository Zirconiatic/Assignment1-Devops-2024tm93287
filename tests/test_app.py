...
def client():
    ...
    with app.test_client() as c:
        # reset in-memory store per test session
...
def test_list_workouts(client):
    client.post("/workouts", data=json.dumps({"workout": "Cycling", "duration": 45}), content_type="application/json")
...
def test_missing_fields(client):
    resp = client.post("/workouts", data=json.dumps({"workout": ""}), content_type="application/json")
    assert resp.status_code == 400
    assert "Please enter both workout and duration." in resp.get_json()["error"]

def test_invalid_duration(client):
    resp = client.post("/workouts", data=json.dumps({"workout": "Yoga", "duration": "abc"}), content_type="application/json")
    assert resp.status_code == 400
